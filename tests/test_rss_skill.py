...
class TestRssReadSkill:
    ...
    async def test_read_feed_success(self):
        ...
        with patch("skills.rss.feedparser.parse", return_value=mock_feed):
    ...
    async def test_read_feed_default_limit(self):
        ...
        mock_feed = MagicMock()
        mock_feed.bozo = False
        mock_feed.feed.get = lambda k, d=None: {"title": "Big Feed"}.get(k, d)
        mock_feed.entries = [
    ...
    async def test_read_feed_bozo_with_entries(self):
        ...
        mock_feed = MagicMock()
        mock_feed.bozo = True
        mock_feed.feed.get = lambda k, d=None: {"title": "Partial Feed"}.get(k, d)
        mock_feed.entries = [
